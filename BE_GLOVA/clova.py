...
# Placeholder for ClovaStudio API
...
async def save_question(user_question: UserQuestion, request: Request):
    ...
    try:
        ...
        # 응답 데이터에서 title, description만 추출출
...
@app.get("/api/result_txt", response_model=ClovaResponse)
...
@app.get("/api/loading", response_model=KeywordResponse)
...
@app.get("/api/result_img", response_model=ImageResponse)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("clova:app", host="0.0.0.0", port=8000, reload=True)
